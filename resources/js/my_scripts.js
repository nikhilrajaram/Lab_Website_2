...
function loadStatsPage() {
	...
	for (var i = 2; i < table.rows.length; i++) {
		...
		var oppName = row.cells[1].innerHTML;
		if (parseInt(row.cells[2].innerHTML) > parseInt(row.cells[3].innerHTML)) {
			row.cells[4].innerHTML = "CU Boulder";
		} else if (parseInt(row.cells[2].innerHTML) < parseInt(row.cells[3].innerHTML)) {
			row.cells[4].innerHTML = oppName;
		} else {
			row.cells[4].innerHTML = "Tie";
		}
	}
}
